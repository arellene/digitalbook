...
<head>
    ...
    <title>Beranda — Pojok Baca (v.B)</title>
    ...
    <div class="content">
        ...
        <div class="anim anim-d3">
            <div class="section-head">
                ...
                <a href="katalog_ebook.php" class="see-all">Lihat Semua <?= icon('arrow-right', 14) ?></a>
            </div>

            <div class="books-grid">
                <?php foreach ($featured as $i => $buku):
                    $c = $covers[$i % count($covers)];
                    $ratingVal = (float)($buku['rating'] ?? 0);
                    $rating = number_format($ratingVal, 1);
                    $baca   = number_format((int)($buku['total_baca'] ?? 0));
                ?>
                <a href="katalog_ebook.php?search=<?= urlencode($buku['judul']) ?>" class="book-card">
