<?php
session_start();
require_once '../config/database.php';

if (!isset($_SESSION['role'])) {
    header('Location: ../index.php');
    exit();
}

$isGuest = ($_SESSION['role'] === 'guest');

if (!$isGuest && !is_numeric($_SESSION['user_id'] ?? '')) {
    header('Location: ../index.php');
    exit();
}

$dbOk = isset($conn) && $conn instanceof mysqli;

$user = null;
if (!$isGuest && $dbOk) {
    $uid  = (int) $_SESSION['user_id'];
    $q    = mysqli_query($conn, "SELECT * FROM users WHERE id = $uid LIMIT 1");
    $user = $q ? mysqli_fetch_assoc($q) : null;
}
if (empty($user)) {
    $user = [
        'nama_lengkap' => $_SESSION['nama_lengkap'] ?? ($isGuest ? 'Tamu' : 'Pengguna'),
        'username'     => $_SESSION['username']     ?? ($isGuest ? 'guest' : 'user'),
        'role'         => $_SESSION['role']         ?? 'anggota',
    ];
}

// ── FILTER & SEARCH ──────────────────────────────────────────────────────────
$search   = trim($_GET['search']   ?? '');
$kategori = trim($_GET['kategori'] ?? '');
$sort     = trim($_GET['sort']     ?? 'terbaru');
$page     = max(1, (int)($_GET['page'] ?? 1));
$perPage  = 12;
$offset   = ($page - 1) * $perPage;

$buku         = [];
$totalBuku    = 0;
$kategoriList = [];
$totalEbook   = 0;
$totalKategori = 0;

if ($dbOk) {
    // Stats
    $r = mysqli_query($conn, "SELECT COUNT(*) AS c FROM buku");
    if ($r) $totalEbook = (int) mysqli_fetch_assoc($r)['c'];
    $r = mysqli_query($conn, "SELECT COUNT(DISTINCT kategori) AS c FROM buku WHERE kategori IS NOT NULL");
    if ($r) $totalKategori = (int) mysqli_fetch_assoc($r)['c'];

    // Daftar kategori
    $rk = mysqli_query($conn, "SELECT DISTINCT kategori FROM buku WHERE kategori IS NOT NULL ORDER BY kategori");
    if ($rk) while ($row = mysqli_fetch_assoc($rk)) $kategoriList[] = $row['kategori'];

    // WHERE
    $where = "1=1";
    if ($search !== '') {
        $s = mysqli_real_escape_string($conn, $search);
        $where .= " AND (judul LIKE '%$s%' OR COALESCE(penulis,pengarang,'') LIKE '%$s%')";
    }
    if ($kategori !== '') {
        $k = mysqli_real_escape_string($conn, $kategori);
        $where .= " AND kategori = '$k'";
    }

    $orderBy = match($sort) {
        'populer'  => 'total_baca DESC',
        'rating'   => 'rating DESC',
        'judul'    => 'judul ASC',
        default    => 'id DESC',
    };

    $rc = mysqli_query($conn, "SELECT COUNT(*) AS c FROM buku WHERE $where");
    if ($rc) $totalBuku = (int) mysqli_fetch_assoc($rc)['c'];

    $sql = "SELECT b.id, b.judul, COALESCE(b.penulis,b.pengarang) AS pengarang, b.kategori,
                   COALESCE(b.tahun,b.tahun_terbit) AS tahun_terbit,
                   COALESCE(b.deskripsi,'') AS deskripsi,
                   b.cover_img, b.cover_emoji,
                   COALESCE(ROUND(AVG(u.rating),1), 0) AS rating,
                   b.total_baca
            FROM buku b
            LEFT JOIN ulasan u ON u.buku_id = b.id
            WHERE $where
            GROUP BY b.id
            ORDER BY $orderBy
            LIMIT $perPage OFFSET $offset";
    $rb = mysqli_query($conn, $sql);
    if ($rb) while ($row = mysqli_fetch_assoc($rb)) $buku[] = $row;
}

// ── FALLBACK DATA ─────────────────────────────────────────────────────────────
if (empty($buku) || $totalEbook === 0) {
    $allBuku = [
        ['id'=>1, 'judul'=>'Atomic Habits',           'pengarang'=>'James Clear',           'kategori'=>'Motivasi & Inspirasi',    'tahun_terbit'=>'2022','rating'=>4.9,'total_baca'=>876, 'deskripsi'=>'Panduan membangun kebiasaan kecil yang menghasilkan perubahan luar biasa.'],
        ['id'=>2, 'judul'=>'Sapiens',                 'pengarang'=>'Yuval Noah Harari',      'kategori'=>'Sejarah & Budaya',        'tahun_terbit'=>'2017','rating'=>4.8,'total_baca'=>1203,'deskripsi'=>'Sejarah singkat umat manusia dari zaman purba hingga era modern.'],
        ['id'=>3, 'judul'=>'Rich Dad Poor Dad',       'pengarang'=>'Robert T. Kiyosaki',     'kategori'=>'Bisnis & Ekonomi',        'tahun_terbit'=>'2021','rating'=>4.7,'total_baca'=>1567,'deskripsi'=>'Pelajaran tentang uang, investasi, dan kebebasan finansial.'],
        ['id'=>4, 'judul'=>'Laskar Pelangi',          'pengarang'=>'Andrea Hirata',          'kategori'=>'Novel',                   'tahun_terbit'=>'2005','rating'=>4.9,'total_baca'=>2341,'deskripsi'=>'Kisah inspiratif anak-anak Belitung yang berjuang meraih pendidikan.'],
        ['id'=>5, 'judul'=>'Python untuk Pemula',     'pengarang'=>'Ahmad Rizky',            'kategori'=>'Komputer & Pemrograman',  'tahun_terbit'=>'2023','rating'=>4.6,'total_baca'=>534, 'deskripsi'=>'Panduan lengkap belajar pemrograman Python dari nol.'],
        ['id'=>6, 'judul'=>'Filosofi Teras',          'pengarang'=>'Henry Manampiring',      'kategori'=>'Motivasi & Inspirasi',    'tahun_terbit'=>'2019','rating'=>4.8,'total_baca'=>1876,'deskripsi'=>'Filosofi Stoa untuk menghadapi dunia yang penuh ketidakpastian.'],
        ['id'=>7, 'judul'=>'Bumi',                    'pengarang'=>'Tere Liye',              'kategori'=>'Novel',                   'tahun_terbit'=>'2022','rating'=>4.6,'total_baca'=>923, 'deskripsi'=>'Petualangan seru di dunia paralel bersama Raib dan kawan-kawan.'],
        ['id'=>8, 'judul'=>'Data Science dengan Python','pengarang'=>'Adi Wijaya, M.Kom.',  'kategori'=>'Komputer & Pemrograman',  'tahun_terbit'=>'2023','rating'=>4.5,'total_baca'=>312, 'deskripsi'=>'Analisis data dan machine learning menggunakan Python.'],
        ['id'=>9, 'judul'=>'Sejarah Tuhan',           'pengarang'=>'Karen Armstrong',        'kategori'=>'Sejarah & Budaya',        'tahun_terbit'=>'2018','rating'=>4.4,'total_baca'=>445, 'deskripsi'=>'Perjalanan 4000 tahun konsep ketuhanan dalam agama-agama besar.'],
        ['id'=>10,'judul'=>'Zero to One',             'pengarang'=>'Peter Thiel',            'kategori'=>'Bisnis & Ekonomi',        'tahun_terbit'=>'2020','rating'=>4.6,'total_baca'=>678, 'deskripsi'=>'Cara membangun startup yang benar-benar inovatif.'],
        ['id'=>11,'judul'=>'Laut Bercerita',          'pengarang'=>'Leila S. Chudori',       'kategori'=>'Novel',                   'tahun_terbit'=>'2017','rating'=>4.8,'total_baca'=>1122,'deskripsi'=>'Kisah aktivis yang menghilang di era Orde Baru.'],
        ['id'=>12,'judul'=>'Clean Code',              'pengarang'=>'Robert C. Martin',       'kategori'=>'Komputer & Pemrograman',  'tahun_terbit'=>'2021','rating'=>4.7,'total_baca'=>289, 'deskripsi'=>'Panduan menulis kode yang bersih dan mudah dipelihara.'],
        ['id'=>13,'judul'=>'Dune',                    'pengarang'=>'Frank Herbert',          'kategori'=>'Novel',                   'tahun_terbit'=>'2020','rating'=>4.7,'total_baca'=>765, 'deskripsi'=>'Epik fiksi ilmiah di dunia gurun Arrakis yang penuh intrik.'],
    ];

    if (empty($kategoriList)) {
        $kategoriList = array_unique(array_column($allBuku, 'kategori'));
        sort($kategoriList);
    }
    if ($totalEbook === 0)    $totalEbook    = count($allBuku);
    if ($totalKategori === 0) $totalKategori = count($kategoriList);

    $filtered = array_filter($allBuku, function($b) use ($search, $kategori) {
        $matchSearch = $search === '' || stripos($b['judul'], $search) !== false || stripos($b['pengarang'], $search) !== false;
        $matchKat    = $kategori === '' || $b['kategori'] === $kategori;
        return $matchSearch && $matchKat;
    });

    usort($filtered, function($a, $b) use ($sort) {
        return match($sort) {
            'populer' => $b['total_baca'] - $a['total_baca'],
            'rating'  => ($b['rating'] <=> $a['rating']),
            'judul'   => strcmp($a['judul'], $b['judul']),
            default   => $b['id'] - $a['id'],
        };
    });

    $totalBuku = count($filtered);
    $buku      = array_slice(array_values($filtered), $offset, $perPage);
}

$totalPages = max(1, (int)ceil($totalBuku / $perPage));

$covers = [
    ['#1a1a2e','#16213e','#0f3460'],
    ['#2d1b33','#4a1942','#7b2d8b'],
    ['#0d2137','#0a3d62','#1e88e5'],
    ['#1b2838','#2a475e','#1b8a4f'],
    ['#2c1810','#5c2d0e','#c0392b'],
    ['#1a2332','#233554','#4a90a4'],
    ['#2d2416','#5c4d2c','#d4a853'],
    ['#1f2b1f','#2d4a2d','#4caf50'],
];

$active_menu = "katalog";

// ── SVG ICON HELPER ──────────────────────────────────────────────────────────
function icon($name, $size = 16, $style = '') {
    $s = $style ? " style=\"$style\"" : '';
    $icons = [
        'house'        => '<path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/>',
        'book-open'    => '<path d="M21 4H3a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h18a1 1 0 0 0 1-1V5a1 1 0 0 0-1-1zM3 6h8v12H3V6zm10 12V6h8v12h-8z"/>',
        'book'         => '<path d="M18 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2zM6 4h5v8l-2.5-1.5L6 12V4zm0 16V14l2.5-1.5L11 14v6H6zm12 0h-5v-6l2.5 1.5L18 14v6zm0-8h-5V4h5v8z"/>',
        'tag'          => '<path d="M21.41 11.58l-9-9C12.05 2.22 11.55 2 11 2H4c-1.1 0-2 .9-2 2v7c0 .55.22 1.05.59 1.42l9 9c.36.36.86.58 1.41.58s1.05-.22 1.41-.59l7-7c.37-.36.59-.86.59-1.41s-.23-1.06-.59-1.42zM5.5 7C4.67 7 4 6.33 4 5.5S4.67 4 5.5 4 7 4.67 7 5.5 6.33 7 5.5 7z"/>',
        'tags'         => '<path d="M21.41 11.58l-9-9C12.05 2.22 11.55 2 11 2H4c-1.1 0-2 .9-2 2v7c0 .55.22 1.05.59 1.42l9 9c.36.36.86.58 1.41.58.55 0 1.05-.22 1.41-.59l7-7c.37-.36.59-.86.59-1.41s-.23-1.06-.59-1.42zM5.5 7C4.67 7 4 6.33 4 5.5S4.67 4 5.5 4 7 4.67 7 5.5 6.33 7 5.5 7zm11.77 8.27L13 19.54l-4.27-4.27 4.27-4.27 4.27 4.27z"/>',
        'layers'       => '<path d="M11.99 18.54l-7.37-5.73L3 14.07l9 7 9-7-1.63-1.27-7.38 5.74zM12 16l7.36-5.73L21 9l-9-7-9 7 1.63 1.27L12 16z"/>',
        'history'      => '<path d="M13 3a9 9 0 0 0-9 9H1l3.89 3.89.07.14L9 12H6c0-3.87 3.13-7 7-7s7 3.13 7 7-3.13 7-7 7c-1.93 0-3.68-.79-4.94-2.06l-1.42 1.42A8.954 8.954 0 0 0 13 21a9 9 0 0 0 0-18zm-1 5v5l4.28 2.54.72-1.21-3.5-2.08V8H12z"/>',
        'star'         => '<path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>',
        'lock'         => '<path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zm3.1-9H8.9V6c0-1.71 1.39-3.1 3.1-3.1 1.71 0 3.1 1.39 3.1 3.1v2z"/>',
        'lock-open'    => '<path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6h2c0-1.72 1.38-3.1 3.1-3.1 1.71 0 3.1 1.38 3.1 3.1v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zm-6 9c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/>',
        'user-plus'    => '<path d="M15 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm-9-2V7H4v3H1v2h3v3h2v-3h3v-2H6zm9 4c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>',
        'sign-in'      => '<path d="M11 7L9.6 8.4l2.6 2.6H2v2h10.2l-2.6 2.6L11 17l5-5-5-5zm9 12h-8v2h8c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-8v2h8v14z"/>',
        'sign-out'     => '<path d="M17 7l-1.41 1.41L18.17 11H8v2h10.17l-2.58 2.58L17 17l5-5zM4 5h8V3H4c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h8v-2H4V5z"/>',
        'user'         => '<path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"/>',
        'check-circle' => '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/>',
        'bell'         => '<path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.9 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"/>',
        'bars'         => '<path d="M3 18h18v-2H3v2zm0-5h18v-2H3v2zm0-7v2h18V6H3z"/>',
        'fire'         => '<path d="M13.5.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67zM12 20c-3.31 0-6-2.69-6-6 0-1.53.57-3.05 1.6-4.19.7 1.98 2.53 3.35 4.58 3.35 2.21 0 3.99-1.56 4.34-3.67.99 1.29 1.48 2.87 1.48 4.51 0 3.31-2.69 6-6 6z"/>',
        'arrow-right'  => '<path d="M8.59 16.59L13.17 12 8.59 7.41 10 6l6 6-6 6z"/>',
        'arrow-left'   => '<path d="M15.41 16.59L10.83 12l4.58-4.59L14 6l-6 6 6 6z"/>',
        'search'       => '<path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z"/>',
        'book-reader'  => '<path d="M21 5c-1.11-.35-2.33-.5-3.5-.5-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5S2.45 4.9 1 6v14.65c0 .25.25.5.5.5.1 0 .15-.05.25-.05C3.1 20.45 5.05 20 6.5 20c1.95 0 4.05.4 5.5 1.5 1.35-.85 3.8-1.5 5.5-1.5 1.65 0 3.35.3 4.75 1.05.1.05.15.05.25.05.25 0 .5-.25.5-.5V6c-.6-.45-1.25-.75-2-1zm0 13.5c-1.1-.35-2.3-.5-3.5-.5-1.7 0-4.15.65-5.5 1.5V8c1.35-.85 3.8-1.5 5.5-1.5 1.2 0 2.4.15 3.5.5v11.5z"/>',
        'unlock'       => '<path d="M12 1C8.676 1 6 3.676 6 7h2c0-2.206 1.794-4 4-4s4 1.794 4 4v3H4c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V12c0-1.1-.9-2-2-2h-2V7c0-3.324-2.676-6-6-6zm0 13c1.1 0 2 .9 2 2s-.9 2-2 2-2-.9-2-2 .9-2 2-2z"/>',
        'bookmark'     => '<path d="M17 3H7c-1.1 0-1.99.9-1.99 2L5 21l7-3 7 3V5c0-1.1-.9-2-2-2z"/>',
        'eye'          => '<path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z"/>',
        'pencil'       => '<path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/>',
        'user-lock'    => '<path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-1.5 0-4.5.75-5.99 2.25C7.18 17.38 9.32 18 12 18c.36 0 .71-.02 1.05-.05-.03-.3-.05-.6-.05-.95 0-1.87.85-3.54 2.18-4.67C14.08 12.11 12.63 14 12 14zm6 1c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3zm0 4.5c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5z"/>',
        'compass'      => '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm-5.5-2.5l7.51-3.49L17.5 6.5 9.99 9.99 6.5 17.5zm5.5-6.6c.61 0 1.1.49 1.1 1.1s-.49 1.1-1.1 1.1-1.1-.49-1.1-1.1.49-1.1 1.1-1.1z"/>',
        'sort'         => '<path d="M3 18h6v-2H3v2zM3 6v2h18V6H3zm0 7h12v-2H3v2z"/>',
        'x'            => '<path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/>',
        'home'         => '<path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/>',
    ];
    $path = $icons[$name] ?? '<circle cx="12" cy="12" r="10"/>';
    return "<svg xmlns='http://www.w3.org/2000/svg' width='{$size}' height='{$size}' viewBox='0 0 24 24' fill='currentColor'{$s}>{$path}</svg>";
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Katalog eBook — Pojok Baca</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="../assets/css/anggota/sidebar.css">
    <link rel="stylesheet" href="../assets/css/anggota/katalog_ebook.css">
</head>
<body>

<?php require_once '../includes/anggota/sidebar.php'; ?>

<!-- ══════════════════════════════════════════════════════════
     MAIN
══════════════════════════════════════════════════════════ -->
<div class="main">

    <!-- TOPBAR -->
    <div class="topbar">
        <div class="topbar-left">
            <button class="menu-btn" onclick="toggleSidebar()"><?= icon('bars', 20) ?></button>
            <div>
                <div class="topbar-title">Katalog eBook</div>
                <div class="topbar-breadcrumb">Pojok Baca / <span>Katalog eBook</span></div>
            </div>
        </div>
        <div class="topbar-right">
            <!-- FITUR BARU (MANDALA): user-chip bisa diklik ke halaman Profil -->
            <?php if (!$isGuest): ?>
            <a href="profil.php" class="user-chip" style="text-decoration:none;color:inherit;">
            <?php else: ?>
            <div class="user-chip">
            <?php endif; ?>
                <div class="chip-ava" style="overflow:hidden;">
                    <?php if (!empty($user['foto_profil']) && is_file(__DIR__ . '/../uploads/profil/' . $user['foto_profil'])): ?>
                        <img src="../uploads/profil/<?= htmlspecialchars($user['foto_profil']) ?>?v=<?= time() ?>" alt="" style="width:100%;height:100%;object-fit:cover;">
                    <?php else: ?>
                        <?= strtoupper(substr($user['nama_lengkap'], 0, 1)) ?>
                    <?php endif; ?>
                </div>
                <div>
                    <div class="chip-name"><?= htmlspecialchars(explode(' ', $user['nama_lengkap'])[0]) ?></div>
                    <div class="chip-role"><?= $isGuest ? 'Tamu' : 'Member' ?></div>
                </div>
            <?php if (!$isGuest): ?>
            </a>
            <?php else: ?>
            </div>
            <?php endif; ?>
        </div>
    </div>

    <!-- CONTENT -->
    <div class="content">

        <!-- ── HERO KATALOG ── -->
        <div class="hero anim anim-d1">
            <div class="hero-text">
                <div class="hero-eyebrow">Pojok Baca Digital Library</div>
                <h2>Katalog <span>eBook</span></h2>
                <p>Temukan ribuan judul dari berbagai kategori. Gunakan filter untuk menemukan bacaan yang sempurna.</p>
            </div>
            <div class="hero-deco"><?= icon('search', 90, 'color:rgba(255,255,255,0.18)') ?></div>
        </div>

        <!-- ── STATS ── -->
        <div class="stats anim anim-d2">
            <div class="stat-card">
                <div class="stat-icon gold"><?= icon('book', 22) ?></div>
                <div>
                    <div class="stat-num"><?= number_format($totalEbook) ?></div>
                    <div class="stat-label">Total eBook</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon teal"><?= icon('tags', 22) ?></div>
                <div>
                    <div class="stat-num"><?= number_format($totalKategori) ?></div>
                    <div class="stat-label">Kategori</div>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-icon red"><?= icon('fire', 22) ?></div>
                <div>
                    <div class="stat-num"><?= number_format($totalBuku) ?></div>
                    <div class="stat-label">Hasil Ditemukan</div>
                </div>
            </div>
        </div>

        <!-- ── GUEST BANNER ── -->
        <?php if ($isGuest): ?>
        <div class="guest-banner anim anim-d2">
            <div class="gb-left">
                <div class="gb-icon"><?= icon('lock-open', 28) ?></div>
                <div>
                    <strong>Buka Akses Penuh — Gratis!</strong>
                    <p>Daftar untuk membaca ebook PDF, simpan koleksi, dan dapatkan rekomendasi personal.</p>
                </div>
            </div>
            <a href="../auth/register.php" class="btn-primary">
                <?= icon('user-plus', 16) ?> Daftar Sekarang
            </a>
        </div>
        <?php endif; ?>

        <!-- ── FILTER BAR ── -->
        <div class="anim anim-d3">
            <form method="GET" action="" class="filter-bar">
                <!-- Search -->
                <div class="search-box">
                    <?= icon('search', 16) ?>
                    <input
                        type="text"
                        name="search"
                        placeholder="Cari judul atau penulis..."
                        value="<?= htmlspecialchars($search) ?>"
                        autocomplete="off"
                    >
                </div>

                <!-- Kategori Select -->
                <select name="kategori" class="filter-select">
                    <option value="">Semua Kategori</option>
                    <?php foreach ($kategoriList as $kat): ?>
                    <option value="<?= htmlspecialchars($kat) ?>" <?= $kategori === $kat ? 'selected' : '' ?>>
                        <?= htmlspecialchars($kat) ?>
                    </option>
                    <?php endforeach; ?>
                </select>

                <!-- Sort -->
                <select name="sort" class="filter-select filter-select--sm">
                    <option value="terbaru" <?= $sort === 'terbaru' ? 'selected' : '' ?>>Terbaru</option>
                    <option value="populer" <?= $sort === 'populer' ? 'selected' : '' ?>>Terpopuler</option>
                    <option value="rating"  <?= $sort === 'rating'  ? 'selected' : '' ?>>Rating Tertinggi</option>
                    <option value="judul"   <?= $sort === 'judul'   ? 'selected' : '' ?>>Judul A-Z</option>
                </select>

                <button type="submit" class="btn-search">
                    <?= icon('search', 15) ?> Cari
                </button>

                <?php if ($search !== '' || $kategori !== '' || $sort !== 'terbaru'): ?>
                <a href="katalog_ebook.php" class="btn-reset">
                    <?= icon('x', 14) ?> Reset
                </a>
                <?php endif; ?>
            </form>
        </div>

        <!-- ── KATEGORI PILLS ── -->
        <?php if (!empty($kategoriList)): ?>
        <div class="kategori-pills anim anim-d3">
            <a href="katalog_ebook.php<?= $sort !== 'terbaru' ? '?sort='.$sort : '' ?>"
               class="pill <?= $kategori === '' ? 'active' : '' ?>">
                Semua
            </a>
            <?php foreach ($kategoriList as $kat):
                $q = http_build_query(array_filter(['kategori'=>$kat, 'sort'=>$sort !== 'terbaru' ? $sort : '']));
            ?>
            <a href="katalog_ebook.php?<?= $q ?>"
               class="pill <?= $kategori === $kat ? 'active' : '' ?>">
                <?= htmlspecialchars($kat) ?>
            </a>
            <?php endforeach; ?>
        </div>
        <?php endif; ?>

        <!-- ── RESULT INFO ── -->
        <div class="result-info anim anim-d3">
            <?php if ($search !== '' || $kategori !== ''): ?>
                Menampilkan <strong><?= number_format($totalBuku) ?></strong> hasil
                <?= $search   !== '' ? ' untuk "<strong>'.htmlspecialchars($search).'</strong>"' : '' ?>
                <?= $kategori !== '' ? ' dalam kategori "<strong>'.htmlspecialchars($kategori).'</strong>"' : '' ?>
            <?php else: ?>
                Menampilkan <strong><?= count($buku) ?></strong> dari <strong><?= number_format($totalBuku) ?></strong> eBook
                <?php if ($sort !== 'terbaru'): ?>
                &mdash; diurutkan: <strong><?= match($sort) { 'populer'=>'Terpopuler', 'rating'=>'Rating Tertinggi', 'judul'=>'Judul A-Z', default=>'Terbaru' } ?></strong>
                <?php endif; ?>
            <?php endif; ?>
        </div>

        <!-- ── BOOKS GRID ── -->
        <div class="anim anim-d4">
        <?php if (empty($buku)): ?>
            <div class="empty-state">
                <?= icon('book', 64, 'display:block;margin:0 auto 16px;opacity:0.15') ?>
                <h3>Tidak ada eBook ditemukan</h3>
                <p>Coba ubah kata kunci pencarian atau pilih kategori lain.</p>
                <a href="katalog_ebook.php" class="btn-primary" style="display:inline-flex;margin-top:20px;">
                    <?= icon('x', 14) ?> Hapus Filter
                </a>
            </div>
        <?php else: ?>
            <div class="books-grid">
                <?php foreach ($buku as $i => $b):
                    $c        = $covers[$i % count($covers)];
                    $ratingVal = (float)($b['rating'] ?? 0);
                    $rating   = number_format($ratingVal, 1);
                    $baca     = number_format((int)($b['total_baca'] ?? 0));
                    $href     = $isGuest ? '#' : 'detail_ebook.php?id=' . (int)$b['id'];
                    $onclick  = $isGuest ? 'onclick="showToast(event); return false;"' : '';
                ?>
                <a href="<?= $href ?>" <?= $onclick ?> class="book-card">
                    <!-- Cover -->
                    <div class="book-cover" style="background:linear-gradient(160deg,<?= $c[0] ?>,<?= $c[1] ?>,<?= $c[2] ?>);">
                        <?php if (!empty($b['cover_img'])): ?>
                            <img src="../<?= htmlspecialchars($b['cover_img']) ?>"
                                 alt="<?= htmlspecialchars($b['judul']) ?>"
                                 style="width:100%;height:100%;object-fit:cover;display:block;">
                        <?php else: ?>
                            <svg class="cover-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="rgba(255,255,255,0.22)">
                                <path d="M21 5c-1.11-.35-2.33-.5-3.5-.5-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5S2.45 4.9 1 6v14.65c0 .25.25.5.5.5.1 0 .15-.05.25-.05C3.1 20.45 5.05 20 6.5 20c1.95 0 4.05.4 5.5 1.5 1.35-.85 3.8-1.5 5.5-1.5 1.65 0 3.35.3 4.75 1.05.1.05.15.05.25.05.25 0 .5-.25.5-.5V6c-.6-.45-1.25-.75-2-1zm0 13.5c-1.1-.35-2.3-.5-3.5-.5-1.7 0-4.15.65-5.5 1.5V8c1.35-.85 3.8-1.5 5.5-1.5 1.2 0 2.4.15 3.5.5v11.5z"/>
                            </svg>
                            <span style="font-size:2.7rem;"><?= htmlspecialchars($b['cover_emoji'] ?? '📚') ?></span>
                        <?php endif; ?>
                        <?php if ($ratingVal > 0): ?>
                        <div class="cover-rating">
                            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="#fbbf24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                            <?= $rating ?>
                        </div>
                        <?php endif; ?>
                        <?php if ($isGuest): ?>
                        <div class="cover-lock-badge"><?= icon('lock', 12) ?></div>
                        <?php endif; ?>
                    </div>

                    <!-- Meta -->
                    <div class="book-meta">
                        <div class="book-genre"><?= htmlspecialchars($b['kategori'] ?? '-') ?></div>
                        <div class="book-title"><?= htmlspecialchars($b['judul']) ?></div>
                        <div class="book-author">
                            <?= icon('pencil', 12) ?>
                            <?= htmlspecialchars($b['pengarang'] ?? '-') ?>
                        </div>
                        <div class="book-reads">
                            <?= icon('eye', 12) ?> <?= $baca ?> dibaca
                        </div>
                    </div>
                </a>
                <?php endforeach; ?>
            </div>

            <!-- ── PAGINATION ── -->
            <?php if ($totalPages > 1): ?>
            <div class="pagination">
                <?php
                $qBase = http_build_query(array_filter(['search'=>$search,'kategori'=>$kategori,'sort'=>$sort !== 'terbaru' ? $sort : '']));
                $qBase = $qBase ? '&'.$qBase : '';
                ?>
                <a href="?page=<?= max(1,$page-1).$qBase ?>" class="pg-btn <?= $page <= 1 ? 'disabled' : '' ?>">
                    <?= icon('arrow-left', 16) ?>
                </a>

                <?php
                $start = max(1, $page - 2);
                $end   = min($totalPages, $page + 2);
                if ($start > 1): ?>
                    <a href="?page=1<?= $qBase ?>" class="pg-btn">1</a>
                    <?php if ($start > 2): ?><span class="pg-dots">…</span><?php endif; ?>
                <?php endif; ?>

                <?php for ($p = $start; $p <= $end; $p++): ?>
                <a href="?page=<?= $p.$qBase ?>" class="pg-btn <?= $p == $page ? 'active' : '' ?>">
                    <?= $p ?>
                </a>
                <?php endfor; ?>

                <?php if ($end < $totalPages): ?>
                    <?php if ($end < $totalPages - 1): ?><span class="pg-dots">…</span><?php endif; ?>
                    <a href="?page=<?= $totalPages.$qBase ?>" class="pg-btn"><?= $totalPages ?></a>
                <?php endif; ?>

                <a href="?page=<?= min($totalPages,$page+1).$qBase ?>" class="pg-btn <?= $page >= $totalPages ? 'disabled' : '' ?>">
                    <?= icon('arrow-right', 16) ?>
                </a>
            </div>
            <?php endif; ?>
        <?php endif; ?>
        </div>

    </div><!-- /content -->
</div><!-- /main -->

<!-- TOAST -->
<div class="toast" id="toast">
    <?= icon('user-lock', 18, 'color:var(--accent)') ?>
    <span>Fitur ini membutuhkan akun.
        <a href="../auth/register.php">Daftar</a> atau
        <a href="../index.php">Login</a>.
    </span>
</div>

<script>
    function toggleSidebar() {
        const s = document.getElementById('sidebar');
        const o = document.getElementById('overlay');
        s.classList.toggle('active');
        o.classList.toggle('active');
    }
    function closeSidebar() {
        document.getElementById('sidebar').classList.remove('active');
        document.getElementById('overlay').classList.remove('active');
    }

    let toastTimer;
    function showToast(e) {
        e && e.preventDefault();
        const t = document.getElementById('toast');
        t.classList.add('show');
        clearTimeout(toastTimer);
        toastTimer = setTimeout(() => t.classList.remove('show'), 4000);
    }
</script>
</body>
</html>